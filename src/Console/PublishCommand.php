...
final class PublishCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'chuckui:publish {component}
    ...
    {
        $components = config('chuck-ui.components');
        ...
        $class = str_replace('ChuckUI\\Components\\', '', $component);
        ...
        if ($this->option('view') || ! $this->option('class')) {
            ...
            $publishedView = $this->laravel->resourcePath('views/vendor/chuck-ui/components/'.$view);
        ...
        if ($this->option('class') || ! $this->option('view')) {
            ...
            if ($filesystem->missing($config = $this->laravel->configPath('chuck-ui.php'))) {
                $this->call('vendor:publish', ['--tag' => 'chuck-ui-config']);
            }
